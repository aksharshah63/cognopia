...
$demos = array(
    ...
    'food' => array(
        'label' => esc_html__('Food Delivery', 'pearl')
    ),
    'spa' => array(
        'label' => esc_html__('SPA', 'pearl')
    ),
    'gym' => array(
	...
	<?php if( !empty($auth_code) ):
        ...
        <div class="stm_install__demo_popup">
            ...
            <div class="inner">
                ...
                <?php if (empty(!$current_demo = get_option('stm_layout'))): ?>
                <!-- Demo already installed notification -->
                <div class="demo_exist">
                    <h4><?php esc_html_e('Attention', 'pearl'); ?></h4>
                    <div class="stm_install__demo_popup_close"></div>
                    <div>
                        <p>
                            <i class="stmicon-uniE6A8"></i> <?php printf('The [<a href="%s" target="_blank">%s</a>] layout is already installed on the website.', get_home_url(), $demos[$current_demo]['label']); ?>
                        </p>
                        <p>
                            <?php _e('Please reset your WordPress via the <a href="https://wordpress.org/plugins/wp-reset/" target="_blank">WP Reset plugin</a> to install another layout.', 'pearl'); ?>
                        </p>
                        <p><?php _e('Please note, the resetting process removes all your current settings and data!', 'pearl') ?></p>
                    </div>
                    <div class="demo_exist_button"><?php esc_html_e('Accept', 'pearl'); ?></div>
                </div>
...
<script>
    (function ($) {
        ...
        $(document).ready(function () {
            ...
            $('.demo_exist_button').on('click', function(e) {
                e.preventDefault();
                $('.stm_install__demo_popup .demo_exist').hide();
                $('.stm_install__demo_popup .privacy_policy').show();
            });
        ...
        function show_popup() {
            ...
            $('.stm_install__demo_popup_close').on('click', function (e) {
                e.preventDefault();
                $('.demo_exist').show();
                $('.demo_install, .privacy_policy').hide();
                if (!installation) {
